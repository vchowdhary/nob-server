...
class NotificationNode{
            ...
            if(notif.prev === null){
                ...
                if(notif.next === null)
                {
                    ...
                    .then(async function (notiflist){
                        ...
                        await models.Notification.findOne({where: {id: notif.next}})
                        .then(async function(notif){
                            notif.update({prev: null});
                            console.log(notif);
    ...
    static async sendAll(notifList)
    {
        ...
        if (currNode.prev === null && currNode.next === null)
        {
            ...
            const status = await models.Status.findOne({where: {id: currNode.to}});
            console.log("Status:");
            console.log(status);
            //TO DO: Fix the statuses. 
            await NotificationNode.send(curr, 0);
        }
        else{
            let time = 0;
            while(currNode !== null && currNode !== undefined)
            {
                await NotificationNode.send(curr, time);
                curr = currNode.next;
