...
class StatusManager {
     ...
     */
    async insert(id, status){
        console.log('Attempting insert...');
        try {
            return await models.sequelize.transaction(async() => {
                await models.Status.findOne({ where: { id: id } })
                    .then(function(obj) {
                        if(obj) { // update
                            return obj.update({ status });
                        }
                        return models.Status.create({ id: id, status: status });
                    });
            });
        } catch (err) {
    ...
    async get(id)
    {
        ...
        {
            return await models.sequelize.transaction(async() => {
                return models.Status.find({ where: { id: id } });
            });
