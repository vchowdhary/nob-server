...
class MatchNotificationManager {
    ...
    async sendNotification(to, match_id, matches, attempt, match_notif_id, req)
    {
        console.log(to);
        console.log("Matches:");
        console.log(matches);
        if(attempt < 14 && to !== '' && matches !== []){
                ...
                .then(async (match) => {
                     ...
                     if(match.provider_id === '')
                     {
                        ...
                        messages.push({
                            ...
                            sound: 'default',
                            title: 'Match Request',
                            data: {attempt: 1, match_notif_id: match_notif_id, req: req, from_id: match.requester_id, to_id: to, type: 'match_request', message: match.requester_id + ' is requesting your help with ' + match.service_type + ': ' + match.subject_1 + ', ' + match.subject_2 + ', ' + match.subject_3 + '\nDetails:' + match.details + '\nLocation:' + match.location.lat + ', ' + match.location.lng}
                        });
            ...
            return await models.sequelize.transaction(async(transaction) => {
                ...
                .then((notification) => {
                   ...
                   if(status === 'rejected' || status === 'later'){
                       matches.splice(i, 1);
                       notification.update({matches: matches.join('|'), status: notification.statuses+JSON.stringify(match_name, status)});
                       return this.sendNotification(matches[i], notification.matchID, matches, attempt, id, req);
                   }
                   else if(status === 'missed'){
                        notification.update({matches: matches, status: notification.statuses+JSON.stringify(match_name, status)});
                       return this.sendNotification(matches[(i+1)%5], notification.matchID, matches, attempt, id, req);
                   }
                   else{
                       notification.update({matches: matches, status: notification.statuses+JSON.stringify(match_name, status)});
                       return "done";
        ...
        router.put(
            ...
            async(req, res) => {
                ...
                try {
                    ...
                    const result = await this.update(id, status, match_name, attempt, request);
                    console.log(result);
