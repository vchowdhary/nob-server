            ...
            async function(req, res) {
                ...
                if ('request' in req.query) {
                    ...
                    await match.run(limit, reqobj, false, req.query.provider_id).then(async function(v)
                    {
                        ...
                        var arr = match.matchIDs.join('|');
                        const notif = await models.MatchNotification.create({matches: arr, matchID: req.query.matchID, 
                            message: req.query.provider_id + ' is offering ' + reqobj.type + ': ' + reqobj.subject_1 + ', ' + reqobj.subject_2 + ', ' + reqobj.subject_3 + '\nDetails:' + reqobj.details + '\nLocation:' + reqobj.location.lat + ', ' + reqobj.location.lng, 
                            statuses: "", maxAttempt: 3});
                        console.log("Created notif");
                        console.log(notif);
                        ...
                        messages.push({
                            ...
                            sound: 'default',
                            title: 'Match Offer',
                            data: {isRequest: false, attempt: 1, match_notif_id: notif.id, req: reqobj, from_id: req.query.provider_id, to_id: match.matchIDs[0], type: 'match_offer', message: req.query.provider_id + ' is offering ' + reqobj.type + ': ' + reqobj.subject_1 + ', ' + reqobj.subject_2 + ', ' + reqobj.subject_3 + '\nDetails:' + reqobj.details + '\nLocation:' + reqobj.location.lat + ', ' + reqobj.location.lng}
                        });
