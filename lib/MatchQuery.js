...
class MatchQuery {
    ...
    async _sendRequests(userID, matchIDs)
    {
        for(var i = 0; i < 3; i++)
        {
            ...
            if(matchIDs.length === 0)
            {
                ...
                for (var j = 0; j < Math.min(matchIDs.length, 5); j++)
                {
                    ...
                    messages.push({
                        to: token.token,
                        sound: 'default',
                        body: `${userID} is requesting your help for this service!`,
                    });
        ...
        router.get(
            ...
            async function(req, res) {
                ...
                if ('request' in req.query) {
                    ...
                    {
                        match.matchIDs = v;
                        match._sendRequests(req.query.requester_id, match.matchIDs).then((result) => {
