...
class MatchQuery {
    ...
    constructor(users, user) {
        ...
        Object.defineProperties(this, /** @lends module:MatchQuery# */ {
            ...
            /**
             * The user who intitiates the request/match's profile
             *
             * @readonly
             * @type {module:User#Profile}
             */
            user: { value: user },
    ...
    async run(limit, req, isRequest) {
        ...
        if(req.type === 'tutoring'){
            ...
            var py    = await spawn('python', ['/Users/vanshikachowdhary/Desktop/night-owl-bakery-master/tools/get_matches.py']),
            ...
            return new Promise(function(resolve, reject){
                py.stdout.on('end', function(){
                    ...
                    limit = Math.min(limit, Object.keys(result.id).length);
                    //console.log(limit);
                    var employeeID2s = new Array(limit);
                    for(var i = 0; i < limit; i++)
                    {
                        employeeID2s[i] = result['id']['' + limit - 1 - i];
                    }
        ...
        console.log('running python script');
        var spawn = require('child_process').spawn;
        var py    = await spawn('python', ['/Users/vanshikachowdhary/Desktop/night-owl-bakery-master/tools/get_matches_delivery.py']),
            data = [req, dataArr, histories],
            dataString = '';
        ...
        router.get(
            ...
            async function(req, res) {
                ...
                if ('request' in req.query) {
                    ...
                    const match = new MatchQuery(users, req.query.requester_id);
                    await match.run(limit, reqobj, true).then(function(v)
                    {
                        match.matchIDs = v;
                    });
                    ...
                    const match = new MatchQuery(users, req.query.provider_id);
                    await match.run(limit, offerobj, false).then(function(v)
                    {
                        match.matchIDs = v;
                    });
                    console.log(match.matchIDs);
