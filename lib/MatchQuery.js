...
class MatchQuery {
    ...
    /*async _sendRequests(userID, matchIDs, req, matchID)
    {
        ...
        for(var i = 0; i < 1; i++)
        {
            ...
            if(matchIDs === null)
            {
                ...
                for (var j = 0; j < Math.min(matchIDs.length, 5); j++)
                {
                        ...
                        let id = 0;
                        await models.Notification.findOrCreate({where: {to: matchIDs[j], from: userID, message: userID + ' is requesting ' + req.type + '. Would you like to help?\nDetails: ' + req.details + '\n Location: ' + req.location.lat + ', ' + req.location.lng + '\nCategories: ' + req.subject_1 + ', ' + req.subject_2 + ', ' + req.subject_3, status: null}})
                        .spread(function(notif, created){
                            ...
                            if( created ){
                                ...
                                if(notif.status === 'accepted'){
                                    await models.Match.findOne({where: {id: matchID}}).then(function(match){
                                        match.update({provider_id: matchIDs[j]});
                                    })
                                    return {id: notif.id, match: matchIDs[j]};
                                    
                                }
                                else if(notif.status === 'rejected' || notif.status === 'later'){
                        ...
                        messages.push({
                            to: token.token,
                            sound: 'default',
                            data: {notif_id: id, req: req, from_id: userID, to_id: token.id, type: 'match_request', message: userID + ' is requesting ' + req.type + '. Would you like to help?\nDetails: ' + req.details + '\n Location: ' + req.location.lat + ', ' + req.location.lng + '\nCategories: ' + req.subject_1 + ', ' + req.subject_2 + ', ' + req.subject_3}
                        });
                        ...
                        for (let chunk of receiptIdChunks) {
                            try {
                                ...
                                /*const notification = await models.Notification.findOne({where: {id: id}});
            ...
            async function(req, res) {
                ...
                if ('request' in req.query) {
                    ...
                    await match.run(limit, reqobj, true, req.query.requester_id).then(async function(v)
                    {
                        ...
                        const notif = await models.MatchNotification.create({matches: arr, matchID: req.query.matchID, message: reqobj.user + ' is requesting your help with ' + reqobj.type + ': ' + reqobj.subject_1 + ', ' + reqobj.subject_2 + ', ' + reqobj.subject_3 + '\nDetails:' + reqobj.details + '\nLocation:' + reqobj.location.lat + ', ' + reqobj.location.lng, statuses: ""});
                        ...
                        messages.push({
                            ...
                            sound: 'default',
                            title: 'Match Request',
                            data: {attempt: 1, match_notif_id: notif.id, req: reqobj, from_id: req.query.requester_id, to_id: match.matchIDs[0], type: 'match_request', message: req.query.requester_id + ' is requesting your help with ' + reqobj.type + ': ' + reqobj.subject_1 + ', ' + reqobj.subject_2 + ', ' + reqobj.subject_3 + '\nDetails:' + reqobj.details + '\nLocation:' + reqobj.location.lat + ', ' + reqobj.location.lng}
                        });
